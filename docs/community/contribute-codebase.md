...
dependencies needed for running omnipath-utils (`[tool.poetry.dependencies]` with the
...
developing omnipath-utils using test-driven development. Please make sure that you
...
If you are brand new to omnipath-utils or open-source development, we recommend
...
omnipath-utils is hosted on GitHub, and to contribute, you will need to sign up for a
...
### Create a fork of omnipath-utils

You will need your own fork of omnipath-utilsin order to eventually open a Pull
Request. Go to the omnipath-utils project page and hit the Fork button. Please
...
```bash
git clone https://github.com/your-user-name/omnipath-utils.git
cd omnipath-utils
git remote add upstream https://github.com/omnipath-utils/omnipath-utils.git
git fetch upstream
```

This creates the directory `omnipath-utils` and connects your repository to the
upstream (main project) *omnipath-utils* repository. They have the same name, but
your local repository and fork are separate from the upstream repository.

### Creating a feature branch

Your local `main` branch should always reflect the current state of omnipath-utils
repository. First ensure it's up-to-date with the main omnipath-utils repository.
...
to create a branch called `my-new-feature-for-omnipath-utils`

```bash
git checkout -b my-new-feature-for-omnipath-utils
```

This changes your working branch from `main` to the
`my-new-feature-for-omnipath-utils` branch. Keep any changes in this branch specific
to one bug or feature so it is clear what the branch brings to *omnipath-utils*. You
...
When making changes, follow these omnipath-utils-specific guidelines:
...
Now your code is on GitHub, but it is not yet a part of the omnipath-utils project.
...
1. Write a descriptive title that includes prefixes. omnipath-utils uses a convention for title prefixes, most commonly, `feat:` for features, `fix:` for bug fixes, and `refactor:` for refactoring.
...
branch with changes in the omnipath-utils `main` branch, run:

```shell

    git checkout my-new-feature-for-omnipath-utils
    git fetch upstream
...
```shell
    git push origin my-new-feature-for-omnipath-utils
```
